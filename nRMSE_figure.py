...
def nRMSE_comp_plot(nRMSE_path_1, nRMSE_path_2, invar_keys = ['MD','FA', 'uFA', 'C_c', 'C_MD'], out_dir = None, fig = None, ax = None, annotate = True, save=True, mean_vs_med=False):
    ...
    container_1 = sio.loadmat(nRMSE_path_1)
    container_2 = sio.loadmat(nRMSE_path_2)

    nRMSE_1 = container_1['nRMSE']
    nRMSE_2 = container_2['nRMSE']

    if np.shape(nRMSE_1)[0] == 1 and np.shape(nRMSE_2)[0] == 1:
        boxplot = False
    else:
        boxplot = True
    ...
    if boxplot:
       #---------------------------------------------------------
        ...
        #---------------------------------------------------------
        bp_1 = ax.boxplot(nRMSE_1, positions=positions_1, patch_artist = True, showmeans=True, meanline=False, widths=1.2,\
        ...
        bp_2 = ax.boxplot(nRMSE_2, positions=positions_2, patch_artist = True, showmeans=True, meanline=False, widths=1.2,\
        ...
        legend = ax.legend([bp_1["boxes"][0], bp_2["boxes"][0], bp_1['medians'][0], bp_1['means'][0]], ['NLLS', 'MLP', 'median', 'mean'], loc='upper left', fontsize = MEDIUM_SIZE*.9, frameon = False)
        ...
        if annotate:
            ...
            if not mean_vs_med:
                ...
                for line_1, line_2, median_1, median_2 in zip(caps_1, caps_2, bp_1['medians'], bp_2['medians']):
                    ...
                    text(x_med1+0.031, y1-0.03, '%.3f' % y_med1, fontsize = MEDIUM_SIZE,
                    ...
                    # overlay median value
                    text(x_med2+0.031, y2-0.024, '%.3f' % y_med2, fontsize = MEDIUM_SIZE,
                        horizontalalignment='right', verticalalignment='top') # draw above, right
            else:
                ...
                for line_1, line_2, mean_1, mean_2 in zip(caps_1, caps_2, bp_1['means'], bp_2['means']):
                    ...
                    y1 = line_1.get_xydata()[0,1] # bottom of cap line
                    # overlay mean value
                    text(x_mean1+0.6, y1-0.03, '%.3f' % y_mean1, fontsize = MEDIUM_SIZE,
                        horizontalalignment='right', verticalalignment='top') # draw above, right
                    # get position data for mean line
                    x_mean2, y_mean2 = mean_2.get_xydata()[0]
                    y2 = line_2.get_xydata()[0,1] # bottom of cap line
                    # overlay mean value
                    text(x_mean2+0.6, y2-0.024, '%.3f' % y_mean2, fontsize = MEDIUM_SIZE,
                        horizontalalignment='right', verticalalignment='top') # draw above, right            
    else:
        mean_vs_med_str = ''
        bar_1 = ax.bar(x=positions_1, height=nRMSE_1[0], color='steelblue', label = 'NLLS')
        bar_2 = ax.bar(x=positions_2, height=nRMSE_2[0], color='mediumaquamarine', label = 'MLP')
        if annotate:
            ax.bar_label(bar_1, label_type='edge', fmt='%.3f')           
            ax.bar_label(bar_2, label_type='edge', fmt='%.3f')
        ax.xaxis.set_ticks([0.75 + pos for pos in positions_1], ticklabels)
        ax.xaxis.set_tick_params(length = 0)
        ax.set_ylim(0.00,0.63)
        ax.legend(loc='upper left', fontsize = MEDIUM_SIZE*.9, frameon = False)
    #---------------------------------------------------------
    # plot options:
    # plt.title(title_str)
    plt.ylabel('nRMSE')
    ...
    if save:
        if out_dir is None:
            out_dir = os.path.split(nRMSE_path_1)[0] 
            if not os.path.exists(out_dir):
                makedirs(out_dir)      
        plt.savefig(os.path.join(out_dir, 'nRMSE_comp_plot' + mean_vs_med_str  + '.pdf'), format='pdf', dpi=1200, bbox_inches='tight')
    return fig, ax